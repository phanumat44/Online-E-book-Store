  ...
  try {
    ...
    try {
      ...
      if (err.code !== "ER_DUP_FIELDNAME") {
        console.error(
          "Error adding columns (might already exist):",
          err.message,
        );
      }
    }
...
app.post("/api/forgot-password", async (req, res) => {
  ...
  try {
    ...
    // 5. Send Email
    const resetLink = `${process.env.DOMAIN}/reset-password?token=${token}`; // Assuming frontend is port 5001
...
app.get("/api/products", async (req, res) => {
  try {
    ...
    const [countResult] = await pool.query(
      `SELECT COUNT(*) as count FROM products${filterQuery}`,
      params,
    );
    ...
    const [rows] = await pool.query(
      `SELECT id, title, description, price, image_url, discount_percent, is_discount_active FROM products${filterQuery} ORDER BY id DESC LIMIT ? OFFSET ?`,
      params,
    );
...
app.get("/api/products/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await pool.query(
      "SELECT id, title, description, price, image_url, status, discount_percent, is_discount_active FROM products WHERE id = ?",
      [id],
    );
    if (rows.length === 0)
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const isDiscActive =
        is_discount_active === "true" || is_discount_active === true ? 1 : 0;
...
app.get(
  "/api/admin/products/:id/download",
  authenticateToken,
  isAdmin,
  async (req, res) => {
    try {
      const { id } = req.params;
      const [rows] = await pool.query(
        "SELECT filename, title FROM products WHERE id = ?",
        [id],
      );
      ...
      if (!product.filename)
        return res
          .status(404)
          .json({ error: "No file attached to this product" });
...
app.post("/api/verify-payment", async (req, res) => {
  ...
  try {
    ...
    if (paymentIntent.status === "succeeded") {
      await pool.query(
        'UPDATE orders SET status = "completed" WHERE stripe_payment_intent_id = ?',
        [paymentIntentId],
      );
...
app.get("/api/download/:orderId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!parsedItems || parsedItems.length === 0)
      return res.status(404).send("No items.");

    const firstItemId = parsedItems[0].id;

    const [products] = await pool.query(
      "SELECT filename FROM products WHERE id = ?",
      [firstItemId],
    );
